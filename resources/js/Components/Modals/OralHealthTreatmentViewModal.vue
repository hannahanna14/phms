<template>
    <Dialog 
        :visible="visible" 
        modal 
        :header="'View Oral Health Treatment'"
        :style="{ width: '50rem' }" 
...
try {
    const timerNotifications = useTimerNotifications('oral_health');
    startTimerMonitoring = timerNotifications.startTimerMonitoring;
    stopTimerMonitoring = timerNotifications.stopTimerMonitoring;
} catch (error) {
    console.error('❌ Failed to import oral health timer notifications:', error);
}
...
watch(() => props.visible, (newVisible) => {
    if (newVisible && props.timerStatus?.status === 'active' && props.remainingMinutes > 0 && startTimerMonitoring) {
        console.log('✅ Starting timer monitoring for oral health treatment modal:', props.treatment?.title);
        try {
        ...
        console.log('🛑 Stopping timer monitoring for oral health treatment modal');
        try {
            stopTimerMonitoring();
        } catch (error) {
